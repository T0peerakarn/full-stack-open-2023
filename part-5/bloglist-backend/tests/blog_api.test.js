...
describe('blogs api', () => {
    ...
    describe('creation of a user', () => {
        ...
        test('fails with a proper error if username is too short', async () => {
            ...
            expect(response.body.error).toContain('`username` (`ml`) is shorter than the minimum allowed length (3)')
        }, 30000)
        ...
        test('fails with a proper error if password is too short', async () => {
            ...
            expect(response.body.error).toContain('`password` is shorter than the minimum allowed length (3)')
        }, 30000)
